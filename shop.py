...
# 🏪 ПРЕДМЕТЫ МАГАЗИНА
# Формат предмета:
# - id: уникальный идентификатор (используется в БД)
# - name: отображаемое название с эмодзи
# - type_name: название категории для отображения
# - type_num: номер слота (для сортировки)
...
# 🔍 Вспомогательная функция: получить название предмета по ID
def get_item_name_by_id(item_id: str) -> str:
    """
    Возвращает красивое название предмета по его ID.
    
    Args:
        item_id: Уникальный идентификатор предмета
        
    Returns:
        str: Название предмета с эмодзи, или ID если не найден
    """
...
# 🔍 Вспомогательная функция: получить предмет по ID
def get_item_by_id(item_id: str) -> dict | None:
    """
    Возвращает полный словарь предмета по его ID.
    
    Args:
        item_id: Уникальный идентификатор предмета
        
    Returns:
        dict | None: Словарь предмета или None если не найден
    """
    for category_items in SHOP_ITEMS.values():
        for item in category_items:
            if item["id"] == item_id:
...
# 🔍 Вспомогательная функция: проверить, можно ли экипировать предмет
def is_equippable(item_id: str) -> bool:
    ...
    Returns:
        bool: True если предмет имеет слот экипировки
    """
    item = get_item_by_id(item_id)
    return item is not None and item.get("slot") is not None

# 🔍 Вспомогательная функция: проверить, можно ли применить предмет
def is_usable(item_id: str) -> bool:
    """
    Проверяет, можно ли предмет применить (зелье/свиток).
    
    Args:
        item_id: Уникальный идентификатор предмета
        
    Returns:
        bool: True если предмет можно применить
    """
    item = get_item_by_id(item_id)
