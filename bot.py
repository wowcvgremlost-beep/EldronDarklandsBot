...
✅ ИСПРАВЛЕНО:
- skill_points НЕ сбрасывается при экипировке/снятии/продаже
- SHOP_ITEMS вынесен в shop.py для удобства
...
from shop import SHOP_ITEMS, get_item_name_by_id, get_item_by_id, is_equippable, is_usable
...
async def battle_action(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "surrender":
        ...
        await callback.message.edit_text("🏳️ Ты сдался.\n💰 Золото потеряно.\n❤️ HP не восстановлены.", reply_markup=main_menu_kb(), parse_mode="HTML")
        await state.clear()
        return
    if action == "attack":
        ...
        if enemy_hp <= 0:
            ...
            await callback.message.edit_text(f"🏆 <b>ПОБЕДА!</b>\n\n⚔️ Ты нанёс {player_dmg} урона!\n👹 {enemy['name']} повержен!\n✨ +{enemy['exp']} опыта\n💰 +{enemy['gold']} золота\n\n⚠️ HP не восстановлены: ❤️ {player['hp']}/{player['max_hp']}", reply_markup=main_menu_kb(), parse_mode="HTML")
            await state.clear()
            return
        ...
        if new_hp <= 0:
            ...
            await callback.message.edit_text(f"💀 <b>ПОРАЖЕНИЕ!</b>\n\n👹 {enemy['name']} нанёс {enemy_dmg} урона{skill_used}\nТы пал в бою...\n💰 Всё золото потеряно.\n❤️ Ты воскрешён с полным HP (только после смерти).", reply_markup=main_menu_kb(), parse_mode="HTML")
            await state.clear()
            return
        db.update_player(callback.from_user.id, hp=new_hp)
        battle["enemy_hp"] = enemy_hp
        await state.update_data(battle=battle)
        await callback.message.edit_text(f"⚔️ <b>Ход завершён</b>\n\n👤 Ты нанёс: {player_dmg} урона\n👹 Враг нанёс: {enemy_dmg} урона{skill_used}\n\n👤 Твой HP: {new_hp}/{player['max_hp']}\n👹 Враг HP: {enemy_hp}/{enemy['max_hp']}\n\n<i>Кинь кубик d20 и напиши число (1-20):</i>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🏳️ Сдаться", callback_data="battle_surrender")]]), parse_mode="HTML")
        await state.set_state(BattleState.player_dice)
        return
    if action == "magic":
        if player["mp"] < 5:
...
async def equip_item(callback: types.CallbackQuery):
    ...
    # ✅ ВАЖНО: skill_points НЕ обновляется из new_stats!
    db.update_player(callback.from_user.id, **{k: new_stats[k] for k in ["phys_atk", "stealth_atk", "evasion", "phys_def", "magic_def", "magic_atk", "max_hp", "max_mp", "hp", "mp"]})
...
async def unequip_all_category(callback: types.CallbackQuery):
    ...
    if len(parts) < 3:
        ...
        return
    category = parts[2]
    slot_map = {"weapons": ["weapon_1", "weapon_2"], "armor": ["armor_1", "armor_2", "armor_3", "armor_4", "armor_5", "armor_6"], "accessories": ["accessory_1", "accessory_2", "accessory_3"]}
