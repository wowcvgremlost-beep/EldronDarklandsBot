...
def create_player(uid, uname, name, race, cls):
    ...
    patk, satk, eva = 5+b["strength"]*4, 10+b["agility"]*11, 8+b["agility"]*3
...
def get_player(uid):
    for att in range(5):
        try:
            with get_connection() as conn:
                c=conn.cursor(); c.execute("SELECT * FROM players WHERE user_id=?",(uid,)); r=c.fetchone()
                if r:
                    p=dict(r)
    ...
    for att in range(5):
        try:
            with get_connection() as conn:
                ...
                sc=", ".join([f"{k}=?" for k in kw]); c.execute(f"UPDATE players SET {sc} WHERE user_id=?",list(kw.values())+[uid])
...
def add_gold(uid,amt):
    for att in range(5):
        try:
            with get_connection() as conn:
                c=conn.cursor(); c.execute("UPDATE players SET gold=gold+? WHERE user_id=?",(amt,uid)); conn.commit()
                return c.rowcount>0
        except:
            if att==4: raise
            time.sleep(att+1)
...
def spend_gold(uid,amt):
    for att in range(5):
        try:
            with get_connection() as conn:
                c=conn.cursor(); c.execute("UPDATE players SET gold=gold-? WHERE user_id=? AND gold>=?",(amt,uid,amt)); conn.commit()
                return c.rowcount>0
        except:
            if att==4: raise
            time.sleep(att+1)
...
def add_log(uid,act,det):
    for att in range(5):
        try:
            with get_connection() as conn:
                c=conn.cursor(); c.execute("INSERT INTO logs(user_id,action,details)VALUES(?,?,?)",(uid,act,det)); conn.commit()
            break
        except:
            if att==4: raise
            time.sleep(att+1)
...
def get_logs(uid,lim=10):
    for att in range(5):
        try:
            with get_connection() as conn:
                c=conn.cursor(); c.execute("SELECT*FROM logs WHERE user_id=? ORDER BY timestamp DESC LIMIT ?",(uid,lim))
        ...
        for cat,items in shop.items():
            for it in items:
                if it["id"]==iid and it.get("stat") in bn:
                    bn[it["stat"]]+=it.get("value",0); break
    return bn

def apply_equip_bonuses(p,shop):
    eb=calc_equip_bonuses(p.get("equipment",{}),shop)
    bs={"strength":p["strength"]-eb["strength"],"vitality":p["vitality"]-eb["vitality"],"agility":p["agility"]-eb["agility"],"intelligence":p["intelligence"]-eb["intelligence"]}
    for k in eb: p[k]=bs[k]+eb[k]
    p["phys_atk"]=5+p["strength"]*4; p["stealth_atk"]=10+p["agility"]*11; p["evasion"]=8+p["agility"]*3
    p["phys_def"]=3+p["vitality"]; p["magic_def"]=3+p["vitality"]; p["magic_atk"]=10+p["intelligence"]*4
    p["max_hp"]=30+p["vitality"]*10; p["max_mp"]=10+p["intelligence"]*3
    return p
